...
def render_compare_selection_page():
    ...
    if uploaded_file is not None:
        ...
        display_width = int(width * 0.3)

        st.image(uploaded_file, caption="Uploaded Image for Comparison", width=display_width)
        st.write("")
        ...
        if st.button("Compare Models"):
            with st.spinner(f"Comparing {model1_name} and {model2_name} predictions..."):
                try:
                    ...
                    predict_url = f"{FASTAPI_URL}/compare"
                    ...
                    st.image(combined_img, caption="Combined Detections (Model 1: Red, Model 2: Green/Blue)", width=int(combined_img.width * 0.3))
    ...
    if st.session_state.compare_predictions and st.session_state.compare_page_view == 'selection':
        ...
        with col_pdf:
            if st.session_state.compare_metrics_df_for_pdf is not None:
                pdf_bytes = utils.create_prediction_pdf(
